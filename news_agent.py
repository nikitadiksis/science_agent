...
def build_rewrite_prompt(title, summary):
    return f"""Перепиши новость для Telegram-канала о науке и технологиях.

Формат:
- первая строка — короткий заголовок с главным фактом
- далее пустая строка
- дальше выбери лучший формат под саму новость:
  1. короткий абзац на 2–4 предложения, если новость лучше объяснять связным текстом
  2. 3–7 буллетов, если новость состоит из отдельных фактов, цифр и пунктов
  3. смешанный формат: 1 короткий вводный абзац и затем 2–5 буллетов, если нужен и контекст, и фактура
- не используй HTML

Главное правило:
Не загоняй каждую новость в буллеты. Выбирай форму, в которой текст читается естественно.
Если буллеты не улучшают подачу, пиши абзацем.

Контекст:
- если в новости фигурирует компания, продукт, лаборатория, фонд или организация, и из исходных данных можно понять кто это или чем они занимаются, обязательно добавь это в начале текста одной короткой фразой
- этот контекст должен помогать читателю, который впервые видит это название
- не выдумывай описание, если его нет в исходных данных

Если выбираешь буллеты:
- количество буллетов зависит только от количества фактов в исходных данных
- не добавляй буллеты ради объёма
...
Если новость основана на исследовании, эксперименте или тестировании модели:
- обязательно укажи ограничения, если они есть в исходных данных
- не делай абсолютных утверждений

Если из исходных данных нельзя честно собрать ни цельный короткий абзац, ни минимум 3 фактических буллета без воды и повторов — верни ровно:
NOT_ENOUGH_FACTS
...
def rewrite_v2(title, summary):
    prompt = build_rewrite_prompt(title, summary)
...
def rewrite(title, summary):
    return rewrite_v2(title, summary)
...
Если новость основана на исследовании, эксперименте или тестировании модели:
- обязательно укажи ограничения (например: тест на текстовых данных, не на реальных условиях)
- не делай абсолютных утверждений
